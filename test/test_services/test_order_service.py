...
def test_get_restaurant_from_cart(mocker):
    """tests that get_restaurant_from_cart will return a restaurant id if given valid input"""
    ...
    result = get_restaurant_from_cart(mock_cart)
    assert result == 10
...
def test_get_restaurant_from_cart(mocker):
    ...
    with pytest.raises(HTTPException) as testException: get_restaurant_from_cart(mock_cart)
    assert testException.value.status_code ==400
