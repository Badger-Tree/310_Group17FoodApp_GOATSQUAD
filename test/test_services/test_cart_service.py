from app.services.cart_service import add_to_cart, clear_cart, create_cart, get_cart_by_customer, calculateSubtotal, delete_from_cart, update_cart
...
def test_clear_cart_valid(mocker): 
    """Tests that clearing a cart is successful"""
    mock_current_cart = [
    {
        "customer_id": "2",
        "cart_id": "EXISTS",
    ...
    result = clear_cart("2", "EXISTS")

    assert result["customer_id"] == "2"
    assert result["cart_id"] == "EXISTS" 
    assert result["cart_items"] == []
    assert result["total"] == 0.0



def test_clear_cart_not_found(mocker): 
    """Tests that clearing a cart is successful"""
    mock_current_cart = [
    {
        "customer_id": "2",
