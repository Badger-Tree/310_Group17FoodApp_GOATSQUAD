...
def test_get_address_by_id_success():
...
def test_get_address_by_customer_id_success():
...
def test_get_address_by_customer_id_not_found():
...
def  test_create_address_not_authenticated():
...
def test_update_address_missing_token():
...
def test_update_address_not_authenticated():
    payload = mock_address_update

    with patch("app.routers.address.get_user_from_session", side_effect=HTTPException(status_code=404, detail="User not found")):
                response = client.put("/addresses/update/3", json = payload,headers={"token":"1"})
                assert response.status_code == 404
...
def test_update_address_partial_input():
...
def test_update_address_address_not_found():
